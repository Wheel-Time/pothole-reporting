...
SELECT
    ...
    FROM test_pothole_reporting.pothole p
        LEFT JOIN test_pothole_reporting.pothole_ledger pl ON pl.fk_pothole_id = p.id
    GROUP BY p.id, p.lat, p.lon, p.create_date;

SELECT * FROM test_pothole_reporting.vw_pothole